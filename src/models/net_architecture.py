...
class PhysX_MKS_GhostNet(nn.Module):
    def __init__(self, num_classes=10, width_mult=1.0, use_vlm=False, tiny=False):
        super(PhysX_MKS_GhostNet, self).__init__()
        self.use_vlm = use_vlm
        
        # --- CONFIGURATION FOR TINY MODE ---
        # Standard: 16 filters in CMKS, 256 in Head
        # Tiny:     12 filters in CMKS, 128 in Head
        cmks_c = 12 if tiny else 16
        head_c = 128 if tiny else 256
        
        # Override width_mult if tiny mode is strictly requested, 
        # or trust the user provided width_mult.
        # Recommendation: If tiny=True, use width_mult=0.65
        
        # --- Module A: CMKS Fusion (Slimmed) ---
        ...
        layers = []
        input_channel = cmks_c * 3 # 12*3=36 or 16*3=48
